(function(){
  ...
  function showCard(raw, x, y){
    ...
    card.innerHTML=`<div class="tg-seed-card-head"><div><h3>${escapeHTML(raw)}</h3>${lemma!==raw?`<span class="lemma">原型：${escapeHTML(lemma)}</span>`:'<span class="lemma">种子原型</span>'}${d&&d.phonetic?`<div class="phonetic">/${escapeHTML(d.phonetic)}/</div>`:''}</div><button class="tg-seed-close" type="button" aria-label="关闭">×</button></div><div class="translation">${d?escapeHTML(d.translation):'暂时没有找到合适的内置释义，可以打开种子匣，结合真题原句继续查看。'}</div>${ety}<p class="summary">这粒种子在真题中出现 ${hits.length} 处。打开种子匣后，可以顺着词源小提示与不同释义，查看它在各年份里的原句；点击“返回阅读位置”会回到你刚才看的地方。</p><div class="tg-seed-actions"><a class="tg-seed-action primary" href="${escapeHTML(seedbankHref)}">打开种子匣</a></div>`;
